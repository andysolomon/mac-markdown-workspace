...
- **Acceptance:** unit tests for the controller; `e2e/web-mobile-editing.spec.ts` (iPhone Safari + Android Chrome with a stand-in `visualViewport`) fails on the old layout and passes on the new; simulated iPhone 16 Pro before/after screenshots; confirm on a real iPhone (Safari tab bar in Compact/Bottom, and a Home Screen web app) with `?viewport-debug`.
