...
### Phase 20 — Mobile edit mode: pinned editing viewport
- **Goal:** Entering edit mode on a phone is one calm motion: the formatting bar sits on the keyboard (just above Safari's URL pill on iPhone), the header never leaves the screen, and the note stays scrollable while typing.
- **Deliverables:** `startEditingViewport` (`src/services/editorViewport.ts`) pins `.app-shell` to the visual viewport while the editor is focused — the accessory bar is the last row of the editor column, not a `position: fixed` overlay (iOS 26's fixed-bottom geometry and `innerHeight` disagree with `visualViewport` by 70–100pt depending on pan); predicts the keyboard line one frame after focus (measured height remembered per orientation) so the caret is already clear when iOS decides whether to pan; undoes any pan with `scrollTo(0, 0)`; reveals the caret only when the visible area changes (the old capture-phase scroll listener snapped every scroll back to the caret). URL-pill clearance 72 → 50pt, skipped for Home Screen web apps. Sticky `:hover` tiles on tapped accessory/bottom-bar buttons removed. Opt-in `?viewport-debug` readout (`ViewportProbe`) for on-device calibration.
- **Acceptance:** `e2e/web-mobile-editing.spec.ts` (iPhone Safari + Android Chrome with a stand-in `visualViewport`: bar position before/after the keyboard, pan undone or followed, scroll not fought, formatting keeps focus, remembered keyboard, hardware-keyboard fallback; attaches a keyboard-up screenshot + geometry JSON) fails on the old layout and passes on the new; isolated tests only for the listed failure modes E2E can't reach (native-shell WebView resize, scroll margins — per AGENTS.md); simulated iPhone 16 Pro before/after screenshots; confirm on a real iPhone (Safari tab bar in Compact/Bottom, and a Home Screen web app) with `?viewport-debug`.

## 5. Out-of-scope / deferred
- Nested tags (`#a/b`), note links/backlinks, pinned/archived notes.
- ~~Cloud sync, multi-device~~ — now its own track: [docs/passwordless-vault-sync.md](docs/passwordless-vault-sync.md) (issues #19–#21; Phases A–D landed) continued by [docs/ambient-vault-sync.md](docs/ambient-vault-sync.md) (Phases F–I: resident keys, 8-char pairing codes, ambient push/pull, `SyncBackend` seam for future user-owned plaintext-`.md` storage). Collaboration and per-note lock remain out of scope.
